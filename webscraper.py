...
cleaned_data = cleaned_data.drop(['Injury_Indicator'], axis=1).set_index('First Name')
cleaned_data['FPPG'] = cleaned_data.FPPG.replace(np.NaN, '0')
cleaned_data['Salary'] = cleaned_data.Salary.replace(np.NaN, '0')
cleaned_data['Id'] = cleaned_data.Id.replace(np.NaN, '0')
...
pd.options.display.float_format = '{:.0f}'.format
 #= pd.to_numeric(cleaned_data["Id"],downcast='integer')
